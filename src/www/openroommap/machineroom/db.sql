CREATE TABLE machineroom (machineroomid integer not null primary key autoincrement, name varchar(255) not null, location text not null, purpose text not null, comments text,addedby varchar(255) not null);

create table measurementset(measurementsetid integer not null primary key autoincrement, machineroomid integer not null references machineroom(machineroomid), updatetime int not null, updatedby varchar(255) not null);

CREATE TABLE measurement (measurementid integer not null primary key autoincrement, measurementsetid integer not null references measurementset(measurementsetid), kiloWatt integer not null, observation text);


create table categories(categoryid integer not null primary key autoincrement, name varchar(255) not null, puePercent integer not null);
CREATE TABLE datacategory(datacategoryid integer not null primary key autoincrement, measurementid integer not null references measurement(measurementid), proportionPercent integer not null, categoryid integer not null references categories(categoryid));
...
insert into measurementset(measurementsetid,machineroomid,updatetime,updatedby) values (0,0,strftime('%s','now'),'acr31');
...
insert into measurementset(measurementsetid,machineroomid,updatetime,updatedby) values (1,1,strftime('%s','now'),'acr31');
